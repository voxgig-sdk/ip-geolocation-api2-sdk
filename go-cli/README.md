# ip-geolocation-api2-cli

boru-driven command-line client **and** interactive REPL for the IpGeolocationApi2
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-geolocation-api2-cli)
make build

# 2. See usage (words, entities, env vars)
./ip-geolocation-api2-cli --help

# 3. Provide credentials once, via the environment
export IP_GEOLOCATION_API2_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./ip-geolocation-api2-cli load 1 entity1            # {id:1} shorthand
./ip-geolocation-api2-cli load '{id:1}' entity1       # explicit match map

# 5. Override the API base URL for a single call
IP_GEOLOCATION_API2_BASE=https://api.example.com ./ip-geolocation-api2-cli load 1 entity1

# 6. No arguments -> interactive REPL
./ip-geolocation-api2-cli
ip-geolocation-api2> load 1 entity1
ip-geolocation-api2> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-geolocation-api2-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ip-geolocation-api2-cli load 1 entity1
   ```
...
```sh
./ip-geolocation-api2-cli load 1 entity1          # scalar shorthand for {id:1}
./ip-geolocation-api2-cli load '{id:1}' entity1     # explicit match map
```
...
./ip-geolocation-api2-cli load 1 entity1
```
...
Run with no arguments to open a REPL (prompt `ip-geolocation-api2>`). Each line is
evaluated as its own boru expression:

```text
$ ./ip-geolocation-api2-cli
ip-geolocation-api2> load 1 entity1
ip-geolocation-api2> /help
ip-geolocation-api2> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `entity1`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ip-geolocation-api2-cli`. |
...
(`./ip-geolocation-api2-cli <expr>`) and interactively (the REPL), and expressions compose the
