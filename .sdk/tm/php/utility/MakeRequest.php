...
class IpGeolocationApi2MakeRequest
{
    public static function call(IpGeolocationApi2Context $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new IpGeolocationApi2Response(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new IpGeolocationApi2Response($fetched);
        } else {
