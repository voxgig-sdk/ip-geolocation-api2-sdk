...
// IpGeolocationApi2 SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = IpGeolocationApi2Omni::makeStructRunner(
    ...
    {
        return IpGeolocationApi2Omni::objmodel(IpGeolocationApi2Omni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return IpGeolocationApi2Omni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return IpGeolocationApi2Omni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(IpGeolocationApi2Omni::structfix($now, Struct::undef()))
                        !== json_encode(IpGeolocationApi2Omni::structfix($val, Struct::undef()))) {
