...
module IpGeolocationApi2Omni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if IpGeolocationApi2Omni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = IpGeolocationApi2Omni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| IpGeolocationApi2Omni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || IpGeolocationApi2Omni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = IpGeolocationApi2Spec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = IpGeolocationApi2Error.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { IpGeolocationApi2Omni.errify(err) }
