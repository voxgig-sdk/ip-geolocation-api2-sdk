...
//    only - a typed IpGeolocationApi2Context there would make every ctx
...
final class IpGeolocationApi2Omni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new IpGeolocationApi2Context($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new IpGeolocationApi2Spec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new IpGeolocationApi2Result($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new IpGeolocationApi2Error('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new IpGeolocationApi2Response($clean['response']);
    ...
    public static function err_from_map($m): ?IpGeolocationApi2Error
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new IpGeolocationApi2Error(is_string($code) ? $code : '', $msg);
    }
