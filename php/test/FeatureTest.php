...
// IpGeolocationApi2 SDK feature test
//
...
class FtCtrl extends IpGeolocationApi2Control
{
...
class FtClient
{
    ...
    private IpGeolocationApi2Utility $utility;

    public function __construct(IpGeolocationApi2Utility $utility, array $options)
    {
    ...
    public function get_utility(): IpGeolocationApi2Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IpGeolocationApi2Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new IpGeolocationApi2Utility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (IpGeolocationApi2Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IpGeolocationApi2Config::make_feature($name);
    ...
    private static function buildUrl(IpGeolocationApi2Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IpGeolocationApi2Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IpGeolocationApi2Response($fetched);
        }
    ...
    private function fail(IpGeolocationApi2Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IpGeolocationApi2Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
