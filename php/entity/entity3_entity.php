...
// IpGeolocationApi2 SDK Entity3 entity
...
class Entity3Entity
{
    ...
    {
        $opts = $this->_entopts;
        return new Entity3Entity($this->_client, $opts);
    }

    /**
     * @param Entity3|array $args Entity3 data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = IpGeolocationApi2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Entity3|array The current Entity3 data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Entity3 fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = IpGeolocationApi2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Entity3 fields).
     */
     ...
     *
     * @param Entity3LoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed Entity3LoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Entity3|array The loaded Entity3 as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = IpGeolocationApi2Helpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
