# ip-geolocation-api2-mcp

[MCP](https://modelcontextprotocol.io) server exposing the IpGeolocationApi2 SDK as
two agent tools — `ip-geolocation-api2_list` and `ip-geolocation-api2_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-geolocation-api2-mcp)
make build

# 2. Provide credentials via the environment
export IP_GEOLOCATION_API2_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ip-geolocation-api2 \
  -- /absolute/path/to/ip-geolocation-api2-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ip-geolocation-api2-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ip-geolocation-api2_list: first page of records
{ "entity": "info" }
{ "entity": "info", "query": { } }

// ip-geolocation-api2_load: one record by id
{ "entity": "entity1", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-geolocation-api2-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ip-geolocation-api2 \
     -- "$PWD"/dist/*/ip-geolocation-api2-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ip-geolocation-api2_list` and `ip-geolocation-api2_load` tools now appear
   in new sessions. Ask the agent to *"list info using ip-geolocation-api2"*
   and it calls `ip-geolocation-api2_list` with `{"entity":"info"}`.
...
```sh
export IP_GEOLOCATION_API2_APIKEY=sk_live_xxx            # API key
export IP_GEOLOCATION_API2_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `ip-geolocation-api2_list` tool
...
### Call the `ip-geolocation-api2_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `ip-geolocation-api2_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ip-geolocation-api2_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IP_GEOLOCATION_API2_APIKEY` | API key sent with every request. |
| `IP_GEOLOCATION_API2_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

entity1 | entity2 | entity3 | info

### Smoke test via HTTP (raw JSON-RPC)

```sh
./ip-geolocation-api2-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ip-geolocation-api2_load","arguments":{"entity":"entity1","query":{"id":1}}}}'
```
