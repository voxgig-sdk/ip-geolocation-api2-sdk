...
func NewContext(ctxmap map[string]any, basectx *Context) *Context {
	...
	if c := getCtxProp(ctxmap, "client"); c != nil {
		if sdk, ok := c.(*IpGeolocationApi2SDK); ok {
			ctx.Client = sdk
		}
	}
...
func (ctx *Context) MakeError(code string, msg string) *IpGeolocationApi2Error {
	return NewIpGeolocationApi2Error(code, msg, ctx)
}
