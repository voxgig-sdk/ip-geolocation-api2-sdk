...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("IP_GEOLOCATION_API2_TEST_LIVE") == "TRUE" ||
		os.Getenv("IP_GEOLOCATION_API2_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("IP_GEOLOCATION_API2_TEST_EXPLAIN"); explain != "" {
