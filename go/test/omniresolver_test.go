...
import (
	...
	omni "github.com/voxgig-sdk/ip-geolocation-api2-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IpGeolocationApi2Error); is {
				out := map[string]any{
					"name":    "IpGeolocationApi2Error",
...
func omniCtx(arg any, client *sdk.IpGeolocationApi2SDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IpGeolocationApi2SDK); is {
			client = live
