...
local function make_error_util(ctx, err)
  ...
  local msg = "IpGeolocationApi2SDK: " .. opname .. ": " .. errmsg
  ...
  local sdk_err = IpGeolocationApi2Error.new("", msg, ctx)
  sdk_err.result = ctx.utility.clean(ctx, result)
  sdk_err.spec = ctx.utility.clean(ctx, spec)

  if type(err) == "table" and getmetatable(err) == IpGeolocationApi2Error then
