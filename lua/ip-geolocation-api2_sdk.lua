...
require("ip-geolocation-api2_types")
...
local IpGeolocationApi2SDK = {}
IpGeolocationApi2SDK.__index = IpGeolocationApi2SDK
...
IpGeolocationApi2SDK._make_feature = _make_feature


function IpGeolocationApi2SDK.new(options)
  local self = setmetatable({}, IpGeolocationApi2SDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function IpGeolocationApi2SDK:options_map()
...
function IpGeolocationApi2SDK:get_utility()
  return Utility.copy(self._utility)
end


function IpGeolocationApi2SDK:get_root_ctx()
  return self._rootctx
end


function IpGeolocationApi2SDK:prepare(fetchargs)
...
function IpGeolocationApi2SDK:direct(fetchargs)
...
function IpGeolocationApi2SDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function IpGeolocationApi2SDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "IpGeolocationApi2SDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function IpGeolocationApi2SDK:_raw_request(fetchargs)
  local utility = self._utility
...
function IpGeolocationApi2SDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "IpGeolocationApi2SDK: graphql: " .. msg
    res.graphql = errors
  end
...
function IpGeolocationApi2SDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = IpGeolocationApi2SDK.new(sdkopts)
  sdk.mode = "test"
