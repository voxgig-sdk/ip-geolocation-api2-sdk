...
local Entity1Entity = {}
Entity1Entity.__index = Entity1Entity


function Entity1Entity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, Entity1Entity)
  self._name = "entity1"
  self._client = client
...
function Entity1Entity:get_name()
  return self._name
end


function Entity1Entity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return Entity1Entity.new(self._client, opts)
end
...
function Entity1Entity:mark_deleted()
  self._deleted = true
end


function Entity1Entity:deleted()
  return true == self._deleted
end


function Entity1Entity:data_set(args)
  if args ~= nil then
...
function Entity1Entity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function Entity1Entity:match_set(args)
  if args ~= nil then
...
function Entity1Entity:match_get()
...
function Entity1Entity:stream(action, args, callopts)
...
---@param reqmatch Entity1LoadMatch
---@param ctrl? table
---@return Entity1
---@return string? err
function Entity1Entity:load(reqmatch, ctrl)
  local utility = self._utility
...
function Entity1Entity:_run_op(ctx, post_done)
  local utility = self._utility
