...
//     (\`new IpGeolocationApi2SDK(...)\` / \`IpGeolocationApi2SDK.test(...)\` become
//     \`IpGeolocationApi2SDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'IpGeolocationApi2SDK'
...
const TEST_SEED = {"entity":{"entity1":{"test01":{"id":"test01"}},"entity2":{"test01":{"id":"test01"}},"entity3":{"test01":{"id":"test01"}},"info":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new IpGeolocationApi2SDK(...)\` and any
// \`IpGeolocationApi2SDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+IpGeolocationApi2SDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/IpGeolocationApi2SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
