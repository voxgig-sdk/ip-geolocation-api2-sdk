...
import { IpGeolocationApi2SDK } from '../../..'
...
describe('Entity1Direct', async () => {
  ...
  test('direct-load-entity1', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-entity1', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['IP_GEOLOCATION_API2_TEST_ENTITY1_ENTID']
