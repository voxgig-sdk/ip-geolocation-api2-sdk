# IpGeolocationApi2 SDK test runner
...
class IpGeolocationApi2TestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IpGeolocationApi2TestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IpGeolocationApi2TestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IpGeolocationApi2TestRunner.getenv("IP_GEOLOCATION_API2_TEST_LIVE")
        override = IpGeolocationApi2TestRunner.getenv("IP_GEOLOCATION_API2_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IpGeolocationApi2TestRunner.getenv(key)
        ...
        explain = IpGeolocationApi2TestRunner.getenv("IP_GEOLOCATION_API2_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["IP_GEOLOCATION_API2_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if IpGeolocationApi2TestRunner._test_control is not None:
            return IpGeolocationApi2TestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IpGeolocationApi2TestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IpGeolocationApi2TestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IpGeolocationApi2TestRunner.load_test_control()
...
def load_env_local():
    IpGeolocationApi2TestRunner.load_env_local()


def env_override(m):
    return IpGeolocationApi2TestRunner.env_override(m)


def entity_data(v):
    return IpGeolocationApi2TestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IpGeolocationApi2TestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IpGeolocationApi2TestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IpGeolocationApi2TestRunner.load_test_control()


def live_client_options():
    return IpGeolocationApi2TestRunner.live_client_options()


def live_delay_ms():
    return IpGeolocationApi2TestRunner.live_delay_ms()
