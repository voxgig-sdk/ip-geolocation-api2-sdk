...
class TestEntity3Entity:

    def test_should_create_instance(self):
        testsdk = IpGeolocationApi2SDK.test(None, None)
        ent = testsdk.Entity3(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _entity3_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "entity3." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set IP_GEOLOCATION_API2_TEST_ENTITY3_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        entity3_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.entity3")))
        entity3_ref01_data = None
        if len(entity3_ref01_data_raw) > 0:
        ...
        entity3_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(entity3_ref01_data_dt0_loaded))
        assert entity3_ref01_data_dt0_load_result is not None
        assert entity3_ref01_data_dt0_load_result["id"] == entity3_ref01_data["id"]



def _entity3_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/entity3/Entity3TestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("IP_GEOLOCATION_API2_TEST_ENTITY3_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("IP_GEOLOCATION_API2_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("IP_GEOLOCATION_API2_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("IP_GEOLOCATION_API2_TEST_EXPLAIN") == "TRUE",
        "live": _live,
