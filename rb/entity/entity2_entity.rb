...
class Entity2Entity
  ...
  def make
    opts = @_entopts.dup
    Entity2Entity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IpGeolocationApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Entity2, Hash] the current Entity2 data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IpGeolocationApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Entity2 fields)
  def match_get
  ...
  #
  # @param reqdata [Entity2CreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Entity2, Hash] the created Entity2; raises IpGeolocationApi2Error on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = IpGeolocationApi2Helpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
