...
module IpGeolocationApi2Utilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = IpGeolocationApi2Context.new({}, nil)
    end
    op = ctx.op || IpGeolocationApi2Operation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || IpGeolocationApi2Result.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(IpGeolocationApi2Error) ? err.msg : err.to_s
    ...
    sdk_err = IpGeolocationApi2Error.new("", msg, ctx)
    sdk_err.result = ctx.utility.clean.call(ctx, result)
    sdk_err.spec = ctx.utility.clean.call(ctx, spec)
    sdk_err.code = err.code if err.is_a?(IpGeolocationApi2Error)
