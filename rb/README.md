# IpGeolocationApi2 Ruby SDK

The Ruby SDK for the IpGeolocationApi2 API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Entity1(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.IpGeolocationApi2(nil).load(
  { "id" => "test01" }, nil
)
...
```
rb/
├── IpGeolocationApi2_sdk.rb       -- Main SDK module
