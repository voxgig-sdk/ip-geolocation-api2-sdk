...
class Entity1EntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = entity1_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "entity1." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set IP_GEOLOCATION_API2_TEST_ENTITY1_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    entity1_ref01_data = nil
    if entity1_ref01_data_raw.length > 0
...
def entity1_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "entity1", "Entity1TestData.json")
  ...
  entid_env_raw = ENV["IP_GEOLOCATION_API2_TEST_ENTITY1_ENTID"]
