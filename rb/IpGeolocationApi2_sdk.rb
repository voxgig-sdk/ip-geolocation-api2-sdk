...
class IpGeolocationApi2SDK
  ...
  def initialize(options = {})
    ...
    utility = IpGeolocationApi2Utility.new
    @_utility = utility
    ...
    feature_opts = IpGeolocationApi2Helpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = IpGeolocationApi2Helpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, IpGeolocationApi2Features.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    IpGeolocationApi2Utility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = IpGeolocationApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = IpGeolocationApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => IpGeolocationApi2Error.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = IpGeolocationApi2Helpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = IpGeolocationApi2Error.new(
        "graphql_error", "IpGeolocationApi2SDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = IpGeolocationApi2SDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
